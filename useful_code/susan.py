...
def getMoravecKps(img_path, win_size=3, win_offset=1, nonMax_size=5, nonMaxValue=0, nFeature=-1, thCRF=-1):
    ...
    cv2.imwrite("../res/keymap.jpg", keymap)
    keymap = np.where(keymap < mean_c, 0, keymap)
    cv2.imwrite("../res/keymap_th.jpg", keymap)
    ...
    cv2.imwrite("../res/keymap_nonMax.jpg", keymap)
...
if __name__ == '__main__':
    kps, img = getMoravecKps("../file/8-1_2.jpg", nFeature=300)
    cv2.imwrite("../res/moravec.jpg", img)
    cv2.imshow("img", img)
    cv2.waitKey(0)